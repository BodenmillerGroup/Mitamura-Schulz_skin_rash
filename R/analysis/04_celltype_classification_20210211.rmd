...
```{r}
sce <- readRDS("~/bbvolume/Data/Skin_rash/Rout/data/sce_protein.rds")

# load all subsetted sce object from hierarchichal gating and combine them
path = "~/bbvolume/Data/Skin_rash/Rout/celltype_annotation/"
...
to be deleted
```{r include=FALSE}
path = "data/celltype_annotation/2/"
file.names <- dir(path, pattern =".rds")
...
for(i in 1:length(file.names)){
 ...
 if (i == 1){
    ...
    labelled_sce_2 <- cbind(labelled_sce_2, file)
  }
}

# another round of labeling sample MDR1_leftside because the classifier obviously interpreted far too many pDCs

path = "data/celltype_annotation/3/"
file.names <- dir(path, pattern =".rds")
...
for(i in 1:length(file.names)){
 ...
 if (i == 1){
    ...
    labelled_sce_3 <- cbind(labelled_sce_3, file)
  }
}

path = "data/celltype_annotation/20210211_old_annotation/"
file.names <- dir(path, pattern =".rds")
...
for(i in 1:length(file.names)){
 ...
 if (i == 1){
    ...
    labelled_sce_4 <- cbind(labelled_sce_4, file)
  }
}

colnames(colData(labelled_sce))
colnames(colData(labelled_sce_2))
colnames(colData(labelled_sce_3))
colnames(colData(labelled_sce_4))

# Because multiple rounds of labelling were performed at different times, we will make a selection of colData for data merging
colData(labelled_sce) <- colData(labelled_sce)[,c("ImageNumber","cellID","cytomapper_CellLabel")]
colData(labelled_sce_2) <- colData(labelled_sce_2)[,c("ImageNumber","cellID","cytomapper_CellLabel")]
colData(labelled_sce_3) <- colData(labelled_sce_3)[,c("ImageNumber","cellID","cytomapper_CellLabel")]
colData(labelled_sce_4) <- colData(labelled_sce_4)[,c("ImageNumber","cellID","cytomapper_CellLabel")]

# now we merge the dataset
labelled_sce <- cbind(labelled_sce,labelled_sce_2)
labelled_sce <- cbind(labelled_sce,labelled_sce_3)

# for merging we add the "exprs" slot to the assays
assay(labelled_sce_4,"exprs") <- assay(labelled_sce_4,"asinh")
labelled_sce <- cbind(labelled_sce,labelled_sce_4)
...
```{r heatmap-visualization per celltype_classified,fig.height=15,fig.width=15}
...
```{r saveRDS}
saveRDS(sce, "~/bbvolume/Data/Skin_rash/Rout/data/sce_protein.rds")
...
write.csv(layer_1_dat, file = "~/bbvolume/Data/Skin_rash/Rout/data/layer_1_classification.csv")
```
