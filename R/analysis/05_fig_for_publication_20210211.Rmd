...
This script generates the plots from figures 2, 3 and supplementary 3 in Mitamura, Schulz et al.
...
image_mat <- as.data.frame(read.csv(file = "~/bbvolume/Data/Skin_rash/processed/cpout/Image.csv",stringsAsFactors = FALSE))

# load the data that contains the information of tissue area per image. in this data all objects per image correspond to the area of the tissue. also the data is still 2 x upscaled and needs to be divided by 4!
tissue_mat <-  as.data.frame(read.csv(file = "~/bbvolume/Data/Skin_rash/processed/cpout/Tissue_MaskTissuePrimary.csv",stringsAsFactors = FALSE))
...
pdf("~/bbvolume/Data/Skin_rash/Rout/plots/cellfractions_classified.pdf",height = 10,width = 15)
plotCellFracGroups(sce,CellClass = "celltype_classified",color_by = "sample_type")
dev.off()
```

```{r barplot of cellular densities }
# create table with cellcounts per image
cur_df <- data.frame(ImageNumber = sce$ImageNumber,
                     celltype_classified = sce$celltype_classified,
                     sample = sce$sample)

cur_df <- reshape2::dcast(data = cur_df,formula = "ImageNumber + sample ~ celltype_classified")

# get the image size per image
im_size <- data.frame(ImageNumber = image_mat$ImageNumber,
                      width = image_mat$Width_cellmask,
                      height = image_mat$Height_cellmask)
im_size$size <- im_size$width*im_size$height

# calculate the area of tissue per image
tissue_area_mat <- unique(tissue_mat %>%
  group_by(ImageNumber) %>%
  mutate(tissue_area = sum(AreaShape_Area)/4) %>%
  select(ImageNumber,tissue_area))

# merge the tissue_mat and image_mat data
tissue_mat <- left_join(tissue_area_mat,im_size[,c("ImageNumber","size")],by="ImageNumber")

# melt data to merge with the tissue area and then divide by tissue area
cur_df <- reshape2::melt(data = cur_df,id.vars=c("ImageNumber","sample"),measure.vars=unique(sce$celltype_classified))

# merge the tissue_mat data with the celltype_classified count data
cur_df <- left_join(cur_df,tissue_mat,"ImageNumber")
cur_df$celltype_density <- (cur_df$value/cur_df$tissue_area)*1000000

pdf(file = "~/bbvolume/Data/Skin_rash/Rout/plots/celltype_densities_barplot.pdf",width = 10,height = 8)
cur_df %>%
ggplot()+
  geom_bar(aes(x=sample,y=celltype_density, fill=as.character(variable)),stat="identity")+
...
pdf(file = "~/bbvolume/Data/Skin_rash/Rout/plots/cellfractions_classified_immune.pdf",width = 12,height = 10)
...
pdf("~/bbvolume/Data/Skin_rash/Rout/plots/celltype_fractions_all.pdf", width = 12, height = 8)
  ...
  ggplot()+
    ...
    geom_jitter(aes(x=celltype,y=value, fill=sample_type), position = position_jitterdodge(0.05), lwd=0.6)+
      theme(axis.text.x = element_text(angle = 45, hjust = 1),
            ...
            panel.grid.major.y=element_line(color="grey", size=.3))+
  scale_fill_manual(values = sample_colors)
pdf(file = paste0("~/bbvolume/Data/Skin_rash/Rout/plots/",i,"_classified_clustered.pdf"))
print(p)
dev.off()
}
```
...
pdf("~/bbvolume/Data/Skin_rash/Rout/plots/Marker_expression_celltype_classified_heatmap_with_annotation.pdf",width = 10,height = 10)
...
pdf("~/bbvolume/Data/Skin_rash/Rout/plots/Marker_expression_celltype_classified_clustered_heatmap_with_annotation.pdf",width = 15,height = 10)
...
pdf(file = "~/bbvolume/Data/Skin_rash/Rout/plots/Marker_expr_heatmap_celltypes_classified_clustered_dittoHeatmap.pdf", width = 15, height = 10)
p
dev.off()
```
...
pdf("~/bbvolume/Data/Skin_rash/Rout/plots/Marker_expression_celltype_classified__by_indication_heatmap_with_annotation.pdf",width = 15,height = 10)
Heatmap(plot_mat,top_annotation = row_ha,height = unit(15,"cm"),width = unit(20,"cm"),cluster_columns = FALSE,
        col = colorRamp2(c(-3, 0, 3), c("dark blue", "white", "dark red")),heatmap_legend_param = list(at = c(-3, 0, 3)))
dev.off()
```
...
pdf(file = "~/bbvolume/Data/Skin_rash/Rout/plots/Marker_expr_heatmap_celltypes_classified_by_indication.pdf", width = 10, height = 8)
p
dev.off()
```
...
jpeg("~/bbvolume/Data/Skin_rash/Rout/plots/UMAP.jpeg",width = 2000,height = 3000,units = "px", res = 300)
